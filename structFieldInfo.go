...
func entityPKFieldName(entity IEntityStruct, typeOf *reflect.Type) (string, error) {
	...
	field := dbMap[strings.ToLower(entity.PK())]
...
func sqlRowsValues(rows *sql.Rows, driverValue *reflect.Value, columnTypes []*sql.ColumnType, dbColumnFieldMap map[string]reflect.StructField, exportFieldMap map[string]reflect.StructField, valueOf *reflect.Value, finder *Finder, cdvMapHasBool bool) error {
	...
	for i, columnType := range columnTypes {
		...
		//从缓存中获取列名的field字段
		//Get the field of the column name from the cache
		...
		if dv.IsValid() && dv.InterfaceData()[0] == 0 { // 该字段的数据库值是null,取默认值
			...
			if convertOK {
				...
				tempDriverValue, errGetDriverValue = convertFunc.GetDriverValue(columnType, &typeOf, finder)
				if errGetDriverValue != nil {
					return LogErr("sqlRowsValues-->convert.GetDriverValue异常: " + errGetDriverValue.Error())
				}
				//如果需要类型转换
				if tempDriverValue != nil {
					...
					fieldTempDriverValueMap[fieldValue] = &drvInfo
					continue
				}
			}
	...
	for fieldValue, driverValueInfo := range fieldTempDriverValueMap {
		...
		typeOf := fieldValue.Type()
		rightValue, err := driverValueInfo.convertFunc.ConvertDriverValue(driverValueInfo.columnType, &typeOf, driverValueInfo.tempDriverValue, finder)
		if err != nil {
			return LogErr("sqlRowsValues-->convert.ConvertDriverValue异常 " + err.Error())
		}
		//给字段赋值
...
func sqlRowsValuesFast(rows *sql.Rows, columns []string, dbColumnFieldMap map[string]reflect.StructField, valueOf reflect.Value) error {
	...
	scanErr := rows.Scan(values...)
	if scanErr != nil {
		scanErr = errors.New("rows.Scan异常 " + scanErr.Error())
		LogErr(scanErr)
		return scanErr
	}
	return nil
...
type driverValueInfo struct {
	convertFunc     CustomDriverValueConvert
...
//实现CustomDriverValueConvert接口,扩展自定义类型,例如 达梦数据库text类型,映射出来的是dm.DmClob类型,无法使用string类型直接接收
...
//如果无法获取到structFieldType,例如Map查询,会传入nil
//返回符合接收类型值的指针,指针,指针!!!!
func (dmtext CustomDMText) ConvertDriverValue(columnType *sql.ColumnType, structFieldType reflect.Type, tempDriverValue driver.Value, finder *grm.Finder) (interface{}, error) {
	//类型转换
