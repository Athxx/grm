...
// unmarshaller (ex: Javascript's) will unmarshal JSON numbers to IEEE 754
...
func NewFromString(value string) (Decimal, error) {
	...
	if eIndex != -1 {
		...
		if err != nil {
			if e, ok := err.(*strconv.NumError); ok && e.Err == strconv.ErrRange {
				return Decimal{}, errors.New("can't convert " + value + " to decimal: fractional part too long")
			}
			return Decimal{}, errors.New("can't convert " + value + " to decimal: exponent is not numeric")
		}
		value = value[:eIndex]
		exp = expInt
	}
	...
	for i := 0; i < vLen; i++ {
		if value[i] == '.' {
			if pIndex > -1 {
				return Decimal{}, errors.New("can't convert " + value + " to decimal: too many .s")
			}
			pIndex = i
		}
	}
	...
	if len(intString) <= 18 {
		...
		if err != nil {
			return Decimal{}, errors.New("can't convert " + value + " to decimal")
		}
	...
	if exp < math.MinInt32 || exp > math.MaxInt32 {
		...
		return Decimal{}, errors.New("can't convert " + originalInput + " to decimal: fractional part too long")
	}
...
// or panics if NewFromString would return an error.
//
// Example:
//
...
func (d Decimal) ExpHullAbrham(overallPrecision uint32) (Decimal, error) {
	...
	if d.Abs().Cmp(overflowThreshold) > 0 {
		return Decimal{}, errors.New("over/underflow threshold, exp(x) cannot be calculated precisely")
	}
	...
	if nf > float64(ExpMaxIterations) || math.IsNaN(nf) {
		return Decimal{}, errors.New("exact value cannot be calculated in <=ExpMaxIterations iterations")
	}
	n := int64(nf)
...
func (d *Decimal) UnmarshalJSON(decimalBytes []byte) error {
	...
	if err != nil {
		return errors.New("error decoding string '" + string(decimalBytes) + "': " + err.Error())
	}

	decimal, err := NewFromString(str)
	*d = decimal
	if err != nil {
		return errors.New("error decoding string '" + str + "': " + err.Error())
	}
	return nil
}
...
func (d *Decimal) UnmarshalBinary(data []byte) error {
	...
	if err := d.value.GobDecode(data[4:]); err != nil {
		return fmt.Errorf("error decoding binary %v: %s", data, err.Error())
	}
