...
源码地址:https://github.com/athxx/grm    

官网:[https://grm.cn](https://grm.cn)  
...
grm生产环境使用参考: [UserStructService.go](https://github.com/athxx/readygo/tree/master/permission/permservice)  
...
## 支持国产数据库  
### 达梦(dm)  
配置grm.DBConfig的 DriverName:dm ,DBType:dm  
达梦数据库驱动: [https://github.com/athxx/dm](https://github.com/athxx/dm)  
达梦的text类型会映射为dm.DmClob,string不能接收,需要实现grm.CustomDriverValueConver接口,自定义扩展处理  

### 人大金仓(kingbase)  
配置grm.DBConfig的 DriverName:kingbase ,DBType:kingbase    
...
### 神舟通用(shentong)  
建议使用官方驱动,配置grm.DBConfig的 DriverName:aci ,DBType:shentong  

### 南大通用(gbase)
~~暂时还未找到官方golang驱动,配置grm.DBConfig的 DriverName:gbase ,DBType:gbase~~  
...
```go  
// grm 使用原生的sql语句,没有对sql语法做限制.语句使用Finder作为载体
// 占位符统一使用?,grm会根据数据库类型,自动替换占位符,例如postgresql数据库把?替换成$1,$2...
// grm使用 ctx context.Context 参数实现事务传播,ctx从web层传递进来即可,例如gin的c.Request.Context()
// grm的事务操作需要显式使用grm.Transaction(ctx, func(ctx context.Context) (interface{}, error) {})开启
``` 
...
func newDemoStruct() demoStruct {
	demo := demoStruct{
		//如果Id=="",保存时grm会调用grm.FuncGenerateStringID(),默认时间戳+随机数,也可以自己定义实现方式,例如 grm.FuncGenerateStringID=funcmyId
...
// testgrm 使用原生的sql语句,没有对sql语法做限制.语句使用Finder作为载体
// 占位符统一使用?,grm会根据数据库类型,自动替换占位符,例如postgresql数据库把?替换成$1,$2...
// grm使用 ctx context.Context 参数实现事务传播,ctx从web层传递进来即可,例如gin的c.Request.Context()
// grm的事务操作需要显式使用grm.Transaction(ctx, func(ctx context.Context) (interface{}, error) {})开启
package testgrm
...
func init() {
	...
	//自定义grm日志输出
	//grm.LogCallDepth = 4 //日志调用的层级
	//grm.FuncLogError = myFuncLogError //记录异常日志的函数
	//grm.FuncLogPanic = myFuncLogPanic //记录panic日志,默认使用defaultLogError实现
	//grm.FuncPrintSQL = myFuncPrintSQL //打印sql的函数

	//自定义日志输出格式,把FuncPrintSQL函数重新赋值
	//log.SetFlags(log.LstdFlags)
	//grm.FuncPrintSQL = grm.FuncPrintSQL

	//dbDaoConfig 数据库的配置.这里只是模拟,生产应该是读取配置配置文件,构造DBConfig
	dbDaoConfig := grm.DBConfig{
		...
		MaxIdleConns: 50,
		//MaxLifetime 连接存活秒时间. 默认600(10分钟)后连接被销毁重建.避免数据库主动断开连接,造成死连接.MySQL默认wait_timeout 28800秒(8小时)
		MaxLifetime: 600,
	...
	// 根据dbDaoConfig创建dbDao, 一个数据库只执行一次,第一个执行的数据库为 defaultDao,后续grm.xxx方法,默认使用的就是defaultDao
...
func TestInsert(t *testing.T) {
	...
	//如果全局DefaultTxOptions配置不满足需求,可以在grm.Transaction事务方法前设置事务的隔离级别,
	//例如 ctx, _ := dbDao.BindCtxTxOptions(ctx, &sql.TxOptions{Isolation: sql.LevelDefault, ReadOnly: false}),如果txOptions为nil,使用全局DefaultTxOptions
...
func TestInsertSlice(t *testing.T) {
	...
	_, err := grm.Transaction(ctx, func(ctx context.Context) (interface{}, error) {
		...
		//slice存放的类型是grm.IEntityStruct!!!,golang目前没有泛型,使用IEntityStruct接口,兼容Struct实体类
...
func TestQueryRow(t *testing.T) {
	...
	//finder.Append 第一个参数是语句,后面的参数是对应的值,值的顺序要正确.语句统一使用?,grm会处理数据库的差异
	//in (?) 参数必须有()括号,不能 in ?
...
//TestUpdateFinder 11.通过finder更新,grm最灵活的方式,可以编写任何更新语句,甚至手动编写insert语句
...
func TestOther(t *testing.T) {
	...
	//场景1.多个数据库.通过对应数据库的dbDao,调用BindCtxDBConn函数,把这个数据库的连接绑定到返回的ctx上,然后把ctx传递到grm的函数即可.
	...
	//把新产生的newCtx传递到grm的函数
	list, _ := grm.QueryMap(newCtx, finder, nil)
	fmt.Println(list)
...
func main() {
	...
	//后续正常初始化grm,一定要放到seata mysql 初始化后面!!!
...
// GrmSeataGlobalTx 包装seata的*tm.DefaultGlobalTx,实现grm.ISeataGlobalTx接口
type GrmSeataGlobalTx struct {
	*tm.DefaultGlobalTx
}

// MySeataGlobalTx grm适配seata分布式事务的函数
// 重要!!!!需要配置grm.DBConfig.SeataGlobalTx=MySeataGlobalTx 重要!!!
func MySeataGlobalTx(ctx context.Context) (grm.ISeataGlobalTx, context.Context, error) {
	//获取seata的rootContext
	rootContext := seataContext.NewRootContext(ctx)
	//创建seata事务
	seataTx := tm.GetCurrentOrCreate(rootContext)
	//使用grm.ISeataGlobalTx接口对象包装seata事务,隔离seata-golang依赖
	seataGlobalTx := GrmSeataGlobalTx{seataTx}

	return seataGlobalTx, rootContext, nil
}

//实现grm.ISeataGlobalTx接口
...
##  性能压测
   ...
   grm 1.2.x 版本实现了基础功能,读性能比gorm和xorm快一倍.随着功能持续增加,造成性能下降,目前读性能只快了50%.  
   grm会持续优化改进性能.  
