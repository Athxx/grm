...
更新内容：
 ...
 - 感谢社区贡献,https://grm.cn 官网上线,很丑的logo上线 :).
 - 支持已经存在的数据库连接
...
更新内容：
 ...
 - 感谢@haming123反馈性能问题.grm 1.2.x 版本实现了基础功能,读性能比gorm和xorm快一倍.随着功能持续增加,造成性能下降,目前读性能只快了50%.  
 - 性能优化,去掉不必要的反射 
 - 完善文档,注释
...
更新内容：
 - 完善文档,注释
 - 千行代码,胜他十万,牛气冲天,grm零依赖.(uuid和decimal这两个工具包竟然有1700行代码)
...
更新内容:
 ...
 - 七家公司的同学建议查询和golang sql方法命名保持统一.做了一个艰难的决定,修改grm的部分方法名.全局依次替换字符串即可.
grm.Query(                 替换为    grm.QueryRow(
grm.QuerySlice(          替换为    grm.Query(
grm.QueryMap(          替换为    grm.QueryRowMap(
grm.QueryMapSlice(   替换为    grm.QueryMap(
...
v1.3.7
更新内容：
 - 非常感谢 @zhou- a- xing 同学(八块腹肌的单身少年)的英文翻译,grm的核心代码注释已经是中英双语了.
...
更新内容：
 ...
 - 增加批量保存Struct对象方法
 - 正式支持达梦数据库
 - 基于达梦官方驱动,发布go mod项目 https://github.com/athxx/dm
...
更新内容：
 ...
 - golang版本依赖调整为v1.13
 - 迁移测试用到readygo,grm项目不依赖任何数据库驱动包
...
更新内容：
 ...
 - 测试用例即文档 https://github.com/athxx/readygo/blob/master/test/testgrm/BaseDao_test.go
...
v1.2.6
更新内容：
 - DBConfig 配置区分 DriverName 和 DBType，兼容一种数据库的多个驱动包
...
更新内容：
 ...
 - 简化数据库读写分离实现,暴露grm.FuncReadWriteBaseDao函数属性,用于自定义读写分离策略
 - 精简grm.DBConfig属性,增加PrintSQL属性

v1.2.2
更新内容：
 - 修改NewPage()返回Page对象指针,传递时少写一个 & 符号
 - 取消GetDBConnection()方法,使用BindCtxConnection()方法进行多个数据库库绑定
